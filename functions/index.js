      ...
      if(orderData.status === "closed") {
      	...
      	return itemsRef.get().then((querySnapshot) => {
			...
			querySnapshot.forEach((item) => {
				...
				balanceRef.doc(itemData.userId).collection("history").add({
				...
				balanceRef.doc(orderData.userId).collection("history").add({
					orderId: context.params.orderId,
			...
			const uniqueUsers = Object.keys(uniqueUsersMap);
      		...
      		uniqueUsers.forEach(userId => {
				balanceRef.doc(userId).collection("history").add({
					orderId: context.params.orderId,
					userId: orderData.userId,
...
exports.calculateBalance = functions.firestore
    ...
    .onWrite((change, context) => {
		const userBalanceRef = admin.firestore().collection("balance").doc(context.params.userId);
		...
		return userBalanceHistoryRef.get().then(querySnapshot => {
			...
			querySnapshot.forEach((doc) => {
				...
				balance[historyRecord.userId] = (parseFloat(balance[historyRecord.userId]) || 0) + parseFloat(historyRecord.amount);
			});
...
exports.createProfile = functions.auth.user()
  .onCreate( (user) => {
  return admin.firestore().collection("userProfile").doc(user.uid).set({
    email: user.email,
