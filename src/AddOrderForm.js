...
class AddOrderForm extends Component {
  state = {name: '', price: '', userId: firebase.auth().currentUser.uid }

  onChange = (e, { name, value }) => this.setState({ [name]: value })

  onSubmit = () => {
    this.props.onSubmit(this.state);
    this.setState({ name: '', price: '', userId: firebase.auth().currentUser.uid });
  }
  ...
  render() {
    ...
    return (<Form className="App" onSubmit={this.onSubmit}>
      <Form.Group widths='equal'>
          ...
          <Form.Button icon labelPosition='left'><Icon name='plus square' />Dodaj do zamówienia</Form.Button>
          </Form.Group>
        </Form>
    );
  }
