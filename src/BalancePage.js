  ...
  componentDidMount() {
    ...
    this.amountsRef.onSnapshot((querySnapshot) => {
    	...
    	querySnapshot.forEach((doc) => {
    		amounts.push({userId: doc.id, amount: doc.data().amount});
    	});
	...
	render() {
		...
		const list = this.state.amounts.map(({userId, amount}, index) => <Table.Row><Table.Cell>{index}</Table.Cell><Table.Cell>{userId}</Table.Cell><Table.Cell collapsing textAlign='right'>{amount} zł</Table.Cell></Table.Row>);
	
    const history = this.state.history.map(({userId, orderId, amount, comment}, index) => <Table.Row><Table.Cell>{index}</Table.Cell><Table.Cell>{orderId}</Table.Cell><Table.Cell>{userId}</Table.Cell><Table.Cell>{comment}</Table.Cell><Table.Cell collapsing textAlign='right'>{amount} zł</Table.Cell></Table.Row>);
return (
  <div>
